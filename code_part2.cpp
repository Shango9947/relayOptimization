...
pair<pdd, pdd> findCenter(pdd p1, pdd p2, double jam) {
    pdd l_center = {(p1.first+p2.first)/2.0, (p1.second+p2.second)/2.0};
    double l_length = distance(p1, p2);
    double height = sqrt(pow(jam, 2) - pow(l_length/2.0, 2));
    double den = p1.first - p2.first; double num = p1.second - p2.second;

    double hyp = sqrt(pow(num, 2) + pow(den, 2));

    // trace(num, den, hyp, num/hyp, den/hyp, l_length, height, l_center);

    pdd c1 = {l_center.first + height*(num/hyp), l_center.second - height*(den/hyp)};
    pdd c2 = {l_center.first - height*(num/hyp), l_center.second + height*(den/hyp)};
...
int minDistanceFrom(set<int> &p1, set<int> &p2, int i) {
    double r1 = 1000000.0; double r2 = 1000000.0;
...
void findSafePairs() {
    ...
    for(int i=0;i<Relay.size();i++) {
        for(int j=i+1;j<Relay.size();j++) {
            double apart = distance(i, j);
            // trace(Relay[i].index, Relay[j].index, apart, 2.0*r_jam);
            if(distance(Relay[i].index, Relay[j].index) > 2.0*r_jam) continue; 
            pair<pdd, pdd> two_center = findCenter({Relay[i].x, Relay[i].y}, {Relay[j].x, Relay[j].y}, r_jam);
            // if(Relay[i].index == 2 && Relay[j].index == 20) trace(two_center);
            bool temp = false; 
            for(auto p : Sink) {
                if(distance(p.index, two_center.first) < safe_radius && distance(p.index, two_center.second) < safe_radius) {
                    temp = true; break;
                }
            }
            // trace(Base.index, Base.type, Base.x, Base.y);
            if(!temp) {
                if(distance(Base.index, two_center.first) < safe_radius && distance(Base.index, two_center.second) < safe_radius) {
                    temp = true; 
                }
            }

            if(Relay[i].index == 1 && Relay[j].index == 41) {
                trace(Relay[i].x, Relay[i].y, Relay[j].x, Relay[j].y);
            }

            if(temp) safe_pairs.insert({Relay[i].index, Relay[j].index}); 
        }
    }
}
...
vi findBadPairs(vi &p1, vi &p2, set<int> &discarded) {
    ...
    for(int i=0;i<total_relay;i++) {
        if(Nodes[i].type == 0 || Nodes[i].type == 2) {
            for(auto p : adjacent_list[i]) adjacent.insert(p);
        }
    }
    ...
    for(int i=0;i<p1.size();i++) {
        for(int j=0; j<p2.size();j++) {
            ...
            if(adjacent.find(p1[i]) != adjacent.end() || adjacent.find(p2[j]) != adjacent.end()) continue; 
            if(safe_pairs.find({p1[i], p2[j]}) != safe_pairs.end() || safe_pairs.find({p2[j], p1[i]}) != safe_pairs.end()) continue; 
            if(validRelayPair(p1[i], p2[j]) && connection(p1[i], p2[j], 2.0*r_jam)) {
                // trace(p1[i], p2[j]);
...
set<int> buildMultiplePath (set<int> start) {
    ...
    while(true) {
        ...
        if(path_found.first.size() == 0) return {};
